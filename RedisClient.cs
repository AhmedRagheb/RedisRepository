...
using Sarmady.Rahawan.Engine.CachingHelpers;
...
namespace Sarmady.Rahawan.Engine
{
    public class RedisClient : IRedisClient
    {
        ...
        public CacheEngineResponse<T> RedisRequest<T>(List<int> ids, string key, Func<T, bool?> filterBy = null) 
                where T : class, IDocument<T>
        {
            var response = new CacheEngineResponse<T>();

            foreach (var id in ids)
            {
                var document = this.Get<T>(key + id);
                if (document == null)
                {
                ...
                var skip = filterBy == null ? null : filterBy(document);

                if (document.IsValid() && skip != true)
                {
                    response.FoundItems.Add(document);
                }
                else
                {
                    Remove(key + id);
